...
-record(todo, {status = reminder, who = puppy, text}).
...
wrd_count_char([H | T], X) ->
    case X of
      % if the word is in the map, inc
      #{H := N} ->
          wrd_count_char(T, X#{H := N + 1});
      % else set word to one
      _ ->
          wrd_count_char(T, X#{H => 1})
    end.
...
map_iter_pred(I, Pred) ->
    case maps:next(I) of
      none ->
          none;
      % I2 is the rest of the iterator
      {K, V, I2} ->
          case Pred(K, V) of
            true ->
                {K, V};
            _ ->
                map_iter_pred(I2, Pred)
          end
    end.
